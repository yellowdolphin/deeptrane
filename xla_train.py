...
try:
    # only needed for torch_xla (multicore)
    import torch_xla.core.xla_model as xm
    from config import DotDict
except ModuleNotFoundError:
    # use xm from local name space, passed to _mp_fn
    pass
    ...
    for batch_idx, batch in enumerate(iterable, start=1):
        ...
        with amp_context:
            ...
            if not 'all' in cfg.freeze:
                # RuntimeError: Function AddmmBackward0 returned an invalid gradient at index 0 - expected device xla:1 but got xla:0
                # Not related to scaler, also happens with loss.backward()
                # Does only occur with n_acc > 1
        ...
        if cfg.use_aux_loss:
            ...
            # on multicore torch_xla: Check failed: pjrt_device == pjrt_data->buffer->device()
            # - can be caused by calling loss.item()
            _loss = loss.detach()  # device
            _n = inputs.size(0)
            _n_acc = cfg.n_acc
            xm.master_print("loss.detach():", _loss.device if hasattr(_loss, 'device') else "no device")
            xm.master_print("inputs.size(0):", _n.device if hasattr(_n, 'device') else "no device")
            xm.master_print("cfg.n_acc:", _n_acc.device if hasattr(_n_acc, 'device') else "no device")
            _val = _loss * _n_acc
            loss_meter.update(_val, _n)
...
def multicore_mp_fn(rank, cfg, metadata, wrapped_model, use_fold):
    ...
    print("rank:", rank)
    xm.master_print(f'In multicore_mp_fn, rank {rank} world_size:', xm.xrt_world_size())
    cfg = DotDict(cfg)

    # Include _mp_fn code, step-by-step
