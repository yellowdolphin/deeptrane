...
defaults = dict(mode=4, cval=0, interpolation=1)

cfg = dict(
    train_tfms = alb.Compose([
        alb.RandomResizedCrop(**size, scale=[0.8, 1], ratio=(0.75, 1.333), interpolation=1),
        #alb.ShiftScaleRotate(p=0.75, shift_limit=0.06, scale_limit=0.25, rotate_limit=0, **defaults),  # deprecated
        alb.Affine(p=0.75, scale=(0.75, 1.25), keep_ratio=False, translate_percent=(-0.06, 0.06), rotate=(0, 0), shear=(0, 0), **defaults),
