...
class StatPooling(tf.keras.layers.Layer):
    ...
    def __init__(self, input_shape, stat_channels, mul_channels=1, add_channels=0, squeeze=False, activation=None, **kwargs):
        ...
        self.conv7 = tf.keras.layers.Conv2D(c1, 1, activation=activation)
        self.conv8 = tf.keras.layers.Conv2D(c1, 1, activation=activation)
    ...
    def call(self, x0, inputs):
        ...
        #x = tf.concat([x0, stat], axis=-1, name="statpool_merge") if x0 is not None else stat
        x = stat
        x = self.bn1(x)
        x = self.conv1(x)
        x = self.bn2(x)
        ...
        # 4-skip-4-(skip)merge
        #x = tf.concat([x, x0], axis=-1, name="statpool_merge") if x0 is not None else x
