...
WHEELS_PATH = '/kaggle/input/pytorch-xla-wheels'
...
def install_vm(version, apt_packages, is_root=False):
  dist_bucket = WHEELS_PATH if os.path.exists(WHEELS_PATH) else DIST_BUCKET
  ...
  if dist_bucket.startswith('gs:'):
    ...
    installation_cmds.extend([
      [sys.executable, '-m', 'pip', 'install', torch_whl_path],
      [sys.executable, '-m', 'pip', 'install', torch_xla_whl_path],
      [sys.executable, '-m', 'pip', 'install', torchvision_whl_path],
    ])
  installation_cmds.append(apt_cmd)
